...
export function TopBar({ title, subtitle }: TopBarProps) {
  const { user, darkMode, toggleDarkMode, switchRole } = useApp();
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="relative">
          <button
            ...
            className="flex items-center gap-2 px-3 py-1.5 text-sm bg-primary-50 dark:bg-primary-900/30 text-primary-700 dark:text-primary-300 rounded-lg hover:bg-primary-100 dark:hover:bg-primary-900/50 transition-colors"
          >
                ...
                {roleOptions.map(option => (
                  <button
                    ...
                    className={`w-full text-left px-4 py-2 text-sm hover:bg-secondary-50 dark:hover:bg-secondary-700 transition-colors ${
                      user?.role === option.value ? 'text-primary-600 bg-primary-50 dark:bg-primary-900/20' : 'text-secondary-700 dark:text-secondary-300'
                    }`}
                  >
                    {option.label}
                  </button>
          ...
          <div className="relative">
            ...
            >
              ...
              <div className="hidden sm:block text-left">
                <p className="text-sm font-medium text-secondary-900 dark:text-white">{user.displayName}</p>
              </div>
              ...
              <>
                ...
                <div className="absolute right-0 top-full mt-1 w-48 bg-white dark:bg-secondary-800 rounded-lg shadow-lg border border-secondary-200 dark:border-secondary-700 py-1 z-20">
                  ...
                  <div className="border-t border-secondary-200 dark:border-secondary-700 mt-1 pt-1">
                    <button className="w-full flex items-center gap-2 px-4 py-2 text-sm text-error-600 dark:text-error-400 hover:bg-error-50 dark:hover:bg-error-900/20 transition-colors">
